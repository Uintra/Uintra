...
namespace uCommunity.Events.App_Plugins.Events
{
    public class EventsController : SurfaceController
    {
        ...
        public ActionResult ItemView(EventsOverviewItemModel model)
        {
        ...
        private IEnumerable<EventsOverviewItemModel> GetOverviewItems(IEnumerable<Compent.uCommunity.Core.Events.Event> events)
        {
            var detailsPageUrl = _eventsService.GetDetailsPage().Url;
            foreach (var @event in events)
            {
                var model = @event.Map<EventsOverviewItemModel>();
