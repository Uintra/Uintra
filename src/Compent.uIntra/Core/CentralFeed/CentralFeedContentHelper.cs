    ...
    public class CentralFeedContentHelper : ICentralFeedContentHelper
    {
        ...
        public IEnumerable<CentralFeedTabModel> GetTabs(IPublishedContent currentPage)
        {
            ...
            foreach (var content in GetContents())
            {
                ...
                // TODO: REMOVE 
                var currentActivityType = _activityTypeProvider.Get((int)tabType);

                // ------------

                var canCreate = _permissionsService.IsCurrentUserHasAccess(currentActivityType, IntranetActivityActionEnum.Create);
